...
import { Service, Project, PricingPlan, BlogPost, Booking, ProjectTrack, SupportTicket, Order } from './types';
...
export default function App() {
  ...
  // Secure Online Payment Gateway Modal States
  const [isGatewayOpen, setIsGatewayOpen] = useState(false);
  const [targetItemToBuy, setTargetItemToBuy] = useState<{
    id: string;
    itemName: string;
    itemType: 'plan' | 'service';
    price: number;
  } | null>(null);

  const [paymentGatewayMethod, setPaymentGatewayMethod] = useState<'card' | 'upi' | 'netbanking'>('card');
  const [gatewayCardForm, setGatewayCardForm] = useState({
    number: '4111 5208 9438 4444',
    expiry: '12/28',
    cvc: '741',
    name: 'Ashish Roy'
  });
  const [gatewayUpiForm, setGatewayUpiForm] = useState({
    upiId: 'ashisher579@okaxis'
  });
  const [gatewayNetBankForm, setGatewayNetBankForm] = useState({
    bankName: 'ICICI Bank'
  });

  const [gatewayStep, setGatewayStep] = useState<'input' | 'processing' | 'success' | 'failed'>('input');
  const [gatewayStatusText, setGatewayStatusText] = useState('Initiating secure handshake...');
  const [completedGatewaySteps, setCompletedGatewaySteps] = useState<string[]>([]);
  const [gatewayTransactionResult, setGatewayTransactionResult] = useState<any>(null);
  const [isReceiptPrintSimulated, setIsReceiptPrintSimulated] = useState(false);
  ...
  const fetchDashboardData = async () => {
    setLoadingStates({ projects: true, bookings: true, tickets: true, orders: true });
    try {
      const [resProj, resBook, resTick, resOrd] = await Promise.all([
      ...
      setUserOrders(resOrd);
    } catch (err) {
      console.warn('Backend API connection offline, utilizing standard mock data stores.', err);
    } finally {
      setLoadingStates({ projects: false, bookings: false, tickets: false, orders: false });
    }
  };
  ...
  // Submit complete visual gateway payment logic
  const submitGatewayPayment = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (!targetItemToBuy) return;
    
    setGatewayStep('processing');
    setCompletedGatewaySteps([]);
    setIsReceiptPrintSimulated(false);
    
    const steps = [
      'Establishing 256-bit SSL encrypted tunnel with gate nodes...',
      'Verifying credit routing limits with card networks...',
      'Acquiring standard authorization token consensus...',
      'Finalizing transaction ledger & compiling invoice...'
    ];
    
    // Simulate real visual checkpoint delays for a high-fidelity checkout experience!
    for (let i = 0; i < steps.length; i++) {
      setGatewayStatusText(steps[i]);
      await new Promise((resolve) => setTimeout(resolve, i === 0 ? 800 : i === 1 ? 1100 : i === 2 ? 1000 : 900));
      setCompletedGatewaySteps(prev => [...prev, steps[i]]);
    }
    
    // Prepare payment method labels
    let paymentMethodLabel = 'Credit Card';
    let paymentDetailsLabel = `•••• ${gatewayCardForm.number.slice(-4)}`;
    
    if (paymentGatewayMethod === 'upi') {
      paymentMethodLabel = 'UPI / GooglePay';
      paymentDetailsLabel = gatewayUpiForm.upiId;
    } else if (paymentGatewayMethod === 'netbanking') {
      paymentMethodLabel = 'NetBanking';
      paymentDetailsLabel = gatewayNetBankForm.bankName;
    }
    ...
    try {
      const response = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          itemName: targetItemToBuy.itemName,
          itemType: targetItemToBuy.itemType,
          price: targetItemToBuy.price,
          purchasedBy: 'ashisher579@gmail.com', // Active registered client
      ...
      if (data.success) {
        setGatewayTransactionResult(data.order);
        setGatewayStep('success');
        fetchDashboardData();
        
        // Push a warm message to the chat assistant confirming receipt
        setChatMessages(prev => [...prev, {
          sender: 'agent',
          text: `🎉 Thank you! We have successfully secured your payment for the "${targetItemToBuy.itemName}". Shankar Sharma and our Lead Systems Dev Nisha Patil are organizing kickoff details. Go to your Client Dashboard ("Project Tracking System") to review!`,
          timestamp: new Date()
        }]);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white font-sans overflow-x-hidden relative">
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="lg:hidden flex items-center space-x-2">
              ...
              <button
                ...
                className="p-2 rounded-lg bg-white/5 border border-white hover:bg-white/10 text-white"
              >
                      ...
                      <div className="pt-4 border-t border-white/5 flex flex-col sm:flex-row gap-4 items-center justify-between">
                        <div className="flex flex-wrap gap-1.5">
                          {service.features.map(feat => (
                            <span key={feat} className="px-2 py-0.5 bg-indigo-400/10 text-indigo-300 border border-indigo-500/10 rounded text-[9px] font-bold uppercase animate-pulse">
                              {feat}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                          <button
                            ...
                            className="flex-1 sm:flex-none px-4 py-2 hover:bg-white/10 text-indigo-300 border border-white/10 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer"
                          >
                            Consult
                          </button>
                          
                          <button
                            id={`btn-checkout-service-${service.id}`}
                            onClick={() => {
                              ...
                              const price = servicePrices[service.id] || 1999;
                              setTargetItemToBuy({
                                id: service.id,
                                itemName: `${service.title} (Deposit)`,
                                itemType: 'service',
                                price: price
                              });
                            ...
                            className="flex-1 sm:flex-none px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all shadow-md cursor-pointer border border-indigo-400/30 font-sans"
                          >
              ...
              {PRICING_PLANS.map(plan => (
                ...
                >
                  ...
                  <div className="pt-8 space-y-3">
                    <button
                      id={`btn-gateway-plan-${plan.id}`}
                      onClick={() => {
                        // Extract numeric price (e.g. "$499" -> 499, "$1,499" -> 1499)
                        const parsedPrice = parseInt(plan.price.replace(/[$,]/g, '')) || 499;
                        setTargetItemToBuy({
                          id: plan.id,
                          itemName: plan.name,
                          itemType: 'plan',
                      ...
                      className={`w-full py-3 rounded-xl text-xs font-black transition-all uppercase tracking-widest flex items-center justify-center space-x-1.5 shadow-md cursor-pointer border ${
                        plan.popular 
                          ? 'bg-gradient-to-r from-blue-500 via-indigo-600 to-purple-600 text-white border-indigo-400/40 hover:brightness-110' 
                          : 'bg-indigo-600 text-white border-indigo-400/20 hover:bg-indigo-500'
                      }`}
                    >
                      <CreditCard className="w-3.5 h-3.5" />
                      <span>Instant Secure Purchase</span>
                    </button>

                    <button
                      id={`btn-booking-plan-${plan.id}`}
                      onClick={() => {
                        setBookingForm(prev => ({ ...prev, notes: `Interested in selecting ${plan.name}. Please follow up for scheduling.` }));
                        setIsBookingModalOpen(true);
                      }}
                      className="w-full py-2.5 rounded-xl text-[10px] font-extrabold transition-all uppercase tracking-wider bg-white/5 border border-white/10 hover:bg-white/10 text-indigo-300 cursor-pointer"
                    >
                      Schedule Setup Call
                    </button>
                  </div>
          ...
          <div id="page-dashboard" className="space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                {/* TRANSACTION history ledger component block */}
                <div className="space-y-4 pt-6 border-t border-white/5">
                  <h3 className="text-base font-black uppercase tracking-widest text-indigo-400 flex items-center gap-2">
                    <ShieldCheck className="w-5 h-5 text-indigo-400" />
                    <span>Secure Transactional Ledger</span>
                  </h3>
                  
                  {userOrders.length > 0 ? (
                    <div className="space-y-4">
                      {userOrders.map(order => (
                        <div key={order.id} className="backdrop-blur-md bg-slate-900/60 border border-white/10 rounded-2xl p-5 space-y-4 shadow-lg hover:border-indigo-500/30 transition-all duration-300">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="text-[10px] text-indigo-400 font-extrabold uppercase tracking-widest">{order.itemType} PURCHASE</p>
                              <h4 className="text-sm font-black text-white leading-tight">{order.itemName}</h4>
                              <p className="text-[9px] font-mono text-slate-400 uppercase">TXN ID: {order.transactionId}</p>
                            </div>
                            <span className="px-2.5 py-1 bg-green-500/20 text-green-300 border border-green-500/30 text-[9px] font-black uppercase tracking-widest rounded-md">
                              Completed
                            </span>
                          </div>

                          <div className="flex justify-between items-center text-xs p-3 bg-slate-950/30 rounded-xl border border-white/5 text-slate-300">
                            <div>
                              <p className="text-[9px] text-slate-400 uppercase font-bold">Funding Source</p>
                              <p className="font-medium text-white">{order.paymentMethod}</p>
                              <p className="text-[10px] text-slate-400">{order.paymentDetails}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-[9px] text-slate-400 uppercase font-bold">Price Settled</p>
                              <p className="text-base font-black text-indigo-300 leading-none">${order.price}</p>
                            </div>
                          </div>

                          <div className="flex justify-between items-center text-[10px] text-slate-400 pt-2 border-t border-white/5">
                            <span>Slipped receipt: {new Date(order.createdAt).toLocaleDateString()}</span>
                            <button
                              id={`btn-ledger-receipt-${order.id}`}
                              onClick={() => {
                                setTargetItemToBuy({
                                  id: order.id,
                                ...
                                setGatewayTransactionResult(order);
                                setGatewayStep('success'); // Re-display the successful secure invoice receipt modal!
                                setIsGatewayOpen(true);
                              }}
                              className="px-3 py-1.5 bg-indigo-500/10 hover:bg-indigo-500/25 border border-indigo-500/20 text-indigo-300 font-bold uppercase rounded-lg transition-all"
                            >
                              Download Receipt
                            </button>
                          </div>
                    ...
                    <div className="p-6 bg-white/5 rounded-2xl text-center border border-dashed border-white/10">
                      <p className="text-xs text-slate-400 font-sans">No transaction receipts registered on secure node ledger.</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* ==================== MODAL: MULTI-MODE SECURE PAYMENT CHECKOUT GATEWAY ==================== */}
      {isGatewayOpen && targetItemToBuy && (
        <div id="payment-gateway-layer" className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-lg bg-slate-950/85 overflow-y-auto">
          <div className="backdrop-blur-2xl bg-slate-900 border border-indigo-500/20 shadow-indigo-500/5 shadow-2xl rounded-3xl max-w-xl w-full p-6 sm:p-8 relative space-y-6 animate-fadeIn max-h-[95vh] overflow-y-auto font-sans leading-relaxed text-slate-200">
            
            {/* Close button except when processing transaction */}
            {gatewayStep !== 'processing' && (
              <button 
                id="btn-close-gateway"
                onClick={() => setIsGatewayOpen(false)} 
                className="absolute top-4 right-4 p-2 bg-white/5 rounded-full hover:bg-white/10 text-slate-400 hover:text-white transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            )}
            ...
            <div className="flex items-center space-x-3 pb-4 border-b border-white/5">
              ...
              <div>
                <h3 className="text-lg font-black text-white tracking-tight uppercase">Checkout Gateway</h3>
                <p className="text-[10px] text-slate-400 font-mono">SECURE SSL TERMINALLY ENCRYPTED 256-BIT HANDSHAKE</p>
              </div>
            </div>

            {/* ITEM SUMMARY DETAILS BAR */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-2">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[9px] text-indigo-400 font-extrabold uppercase tracking-widest">{targetItemToBuy.itemType} on boarding</span>
                  <h4 className="text-base font-black text-white leading-tight">{targetItemToBuy.itemName}</h4>
                </div>
                <div className="text-right">
                  <p className="text-[9px] text-slate-400 uppercase font-black">Authorized Fee</p>
                  <p className="text-2xl font-black text-indigo-300">${targetItemToBuy.price}</p>
                </div>
              </div>
              <div className="pt-2 border-t border-white/5 flex justify-between text-[11px] text-slate-400">
                <span>Payer Email: <strong className="text-white">ashisher579@gmail.com</strong></span>
                <span>Category: <strong className="text-white uppercase">{targetItemToBuy.itemType}</strong></span>
              </div>
            </div>

            {/* MAIN STEP 1: CHOOSE AND FILL INPUTS */}
            {gatewayStep === 'input' && (
              <div className="space-y-6">
                
                {/* SELECTOR TABS */}
                <div className="space-y-2">
                  <label className="text-[10px] text-slate-400 uppercase font-black">Choose Funding Instrument</label>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { key: 'card' as const, title: 'Card', icon: CreditCard },
                      { key: 'upi' as const, title: 'UPI App', icon: ShieldCheck },
                      { key: 'netbanking' as const, title: 'NetBanking', icon: Globe }
                    ].map(method => {
                      ...
                      return (
                        <button
                          key={method.key}
                          type="button"
                          id={`btn-gateway-tab-${method.key}`}
                          onClick={() => setPaymentGatewayMethod(method.key)}
                          className={`py-3 px-2 rounded-xl text-xs font-bold uppercase flex flex-col items-center justify-center gap-1.5 transition-all cursor-pointer border ${
                            paymentGatewayMethod === method.key 
                ...
                {/* FORM VIEWS DEPENDING ON TAB */}
                <form onSubmit={(e) => { e.preventDefault(); submitGatewayPayment(); }} className="space-y-4">
                  
                  {/* CREDIT CARD FORM */}
                  {paymentGatewayMethod === 'card' && (
                    <div className="space-y-3 p-4 bg-slate-950/40 border border-white/10 rounded-2xl animate-fadeIn">
                      <div className="space-y-1">
                        <label className="text-[9px] uppercase font-bold text-slate-400">Enter Card Number</label>
                        <div className="relative">
                          <input 
                            type="text" 
                            required
                            placeholder="4111 5208 9438 4444"
                            value={gatewayCardForm.number}
                            onChange={e => setGatewayCardForm({ ...gatewayCardForm, number: e.target.value })}
                            className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:border-indigo-400 focus:outline-none placeholder-slate-600" 
                          />
                          <span className="absolute top-2.5 right-3 px-2 py-0.5 bg-indigo-500/20 text-indigo-200 border border-indigo-500/20 rounded-[4px] font-black font-mono text-[9px] uppercase">VISA</span>
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <label className="text-[9px] uppercase font-bold text-slate-400">Valid Till (MM/YY)</label>
                          <input 
                            type="text" 
                            required
                            placeholder="12/28"
                            value={gatewayCardForm.expiry}
                            onChange={e => setGatewayCardForm({ ...gatewayCardForm, expiry: e.target.value })}
                            className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:border-indigo-400 focus:outline-none placeholder-slate-600" 
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[9px] uppercase font-bold text-slate-400">Security CVC</label>
                          <input 
                            type="password" 
                            required
                            maxLength={4}
                            placeholder="741"
                            value={gatewayCardForm.cvc}
                            onChange={e => setGatewayCardForm({ ...gatewayCardForm, cvc: e.target.value })}
                            className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:border-indigo-400 focus:outline-none placeholder-slate-600" 
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <input 
                          type="text" 
                          required
                          placeholder="Ashish Roy"
                          value={gatewayCardForm.name}
                          onChange={e => setGatewayCardForm({ ...gatewayCardForm, name: e.target.value })}
                          className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:border-indigo-400 focus:outline-none placeholder-slate-600" 
                        />
                      </div>
                    </div>
                  ...
                  {paymentGatewayMethod === 'upi' && (
                    <div className="space-y-4 p-5 bg-slate-950/40 border border-white/10 rounded-2xl animate-fadeIn text-center">
                      <div className="w-20 h-20 bg-white/5 rounded-full border border-indigo-500/25 flex items-center justify-center mx-auto mb-2 text-indigo-400">
                        <ShieldCheck className="w-10 h-10" />
                      </div>
                      
                      <div className="space-y-1 max-w-sm mx-auto">
                        <label className="text-[9px] uppercase font-bold text-slate-400">Virtual Payment Address (VPA)</label>
                        <input 
                          type="text" 
                          required
                          placeholder="ashisher579@okaxis"
                          value={gatewayUpiForm.upiId}
                          onChange={e => setGatewayUpiForm({ ...gatewayUpiForm, upiId: e.target.value })}
                          className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-center text-white focus:border-indigo-400 focus:outline-none placeholder-slate-600" 
                        />
                      </div>
                      <p className="text-[10px] text-slate-400 max-w-xs mx-auto">A payment request notice will instantly transmit securely to your GPay, PhonePe, or BHIM UPI handle.</p>
                    </div>
                  )}

                  {/* NET BANKING FORM */}
                  {paymentGatewayMethod === 'netbanking' && (
                    <div className="space-y-3 p-4 bg-slate-950/40 border border-white/10 rounded-2xl animate-fadeIn">
                      <div className="space-y-1">
                        <label className="text-[9px] uppercase font-bold text-slate-400 font-black">Choose Institutional Bank</label>
                        <select
                          value={gatewayNetBankForm.bankName}
                          onChange={e => setGatewayNetBankForm({ bankName: e.target.value })}
                          className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs focus:border-indigo-400 focus:outline-none text-white"
                        >
                          <option>ICICI Bank</option>
                          <option>HDFC Bank Ltd</option>
                          <option>State Bank of India</option>
                          <option>Chase Corporate Banking</option>
                          <option>HSBC Global Services</option>
                        </select>
                      </div>
                      <p className="text-[10px] text-slate-400 max-w-xs pt-1.5">You will safely re-route to your institution’s credential terminal to authorize the payout.</p>
                    </div>
                  )}

                  {/* PCI GUARANTEE SHIELD BADGING */}
                  <div className="flex items-center space-x-2 justify-center p-3.5 bg-indigo-500/5 rounded-2xl border border-indigo-500/10 text-[10px] text-slate-400">
                    <span className="text-indigo-400">🛡️</span>
                    <span>PCI-DSS Compliant Secure Ledger. SSL encrypted transmission.</span>
                  </div>

                  {/* KICK TRIGGER */}
                  <button
                    type="submit"
                    id="btn-trigger-gateway-submit"
                    className="w-full py-3 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:brightness-110 text-white font-black uppercase text-xs tracking-widest rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 cursor-pointer"
                  >
                    <Lock className="w-4 h-4 text-white" />
                    <span>Authorize Secure Settlement ${targetItemToBuy.price}</span>
                  </button>
            ...
            {gatewayStep === 'processing' && (
              <div className="py-8 space-y-6 text-center animate-fadeIn">
                ...
                <div className="space-y-4">
                  <h4 className="text-base font-black text-white uppercase tracking-widest animate-pulse">Securing Transaction...</h4>
                  <p className="text-xs text-indigo-300 font-mono italic max-w-xs mx-auto">{gatewayStatusText}</p>
                </div>

                {/* Secure checkpoint checkmark list */}
                <div className="max-w-xs mx-auto bg-slate-950/60 border border-white/5 p-4 rounded-2xl space-y-2 text-[10px] text-left">
                  {[
                    'Establishing 256-bit SSL encrypted tunnel with gate nodes...',
                    'Verifying credit routing limits with card networks...',
                    'Acquiring standard authorization token consensus...',
                    'Finalizing transaction ledger & compiling invoice...'
                  ].map((stepString, stepId) => {
                    const isChecked = completedGatewaySteps.includes(stepString);
                    return (
                      <div key={stepId} className="flex items-center space-x-2">
                        <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center text-[8px] font-black ${
                          isChecked ? 'bg-green-500/20 text-green-300' : 'bg-slate-900 border border-white/10 text-slate-500'
                        }`}>
                          {isChecked ? '✓' : '●'}
                        </span>
                        <span className={isChecked ? 'text-slate-300 line-through' : 'text-slate-500 font-mono'}>{stepString.split('...')[0]}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* MAIN STEP 3: TRANSACTION SUCCESS & DIGITAL INVOICE RECEIPT */}
            {gatewayStep === 'success' && gatewayTransactionResult && (
              <div className="space-y-6 animate-scaleIn">
                ...
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-xl font-black text-green-400 tracking-tight uppercase">Payment Secured</h3>
                  <p className="text-xs text-slate-400">The digital ledger entry has been successfully verified.</p>
                </div>

                {/* DETAILED DIGITAL INVOICE */}
                <div className="bg-white text-slate-950 rounded-2xl p-6 space-y-4 shadow-2xl relative overflow-hidden font-sans border-2 border-indigo-500/20">
                  
                  {/* Decorative invoice layout details */}
                  <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none select-none font-bold text-5xl">MARGDARSHAK</div>
                  
                  <div className="flex justify-between items-start pb-4 border-b border-slate-250">
                    <div>
                      <p className="text-[9px] font-black text-indigo-600 uppercase tracking-widest">OFFICIAL RECEIPTS</p>
                      <h4 className="text-base font-black uppercase">Margdarshak IT</h4>
                      <p className="text-[9px] text-slate-500 font-mono">Spectrum Tech Towers, BKC, Mumbai</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-slate-400 uppercase">Receipt No:</p>
                      <p className="text-xs font-mono font-black">{gatewayTransactionResult.id}</p>
                    </div>
                  </div>

                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-bold">Target Item:</span>
                      <span className="font-extrabold text-slate-900">{gatewayTransactionResult.itemName}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-bold font-sans">Settler Payer:</span>
                      <span className="font-bold text-slate-900">{gatewayTransactionResult.purchasedBy}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-bold font-sans">Method Code:</span>
                      <span className="font-bold text-slate-900">{gatewayTransactionResult.paymentMethod}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-bold font-sans">Method Details:</span>
                      <span className="font-mono text-slate-900 font-bold">{gatewayTransactionResult.paymentDetails}</span>
                    </div>
                    <div className="flex justify-between font-mono">
                      <span className="text-slate-500">Invoice Slipped:</span>
                      <span className="text-slate-900 font-bold">{new Date(gatewayTransactionResult.createdAt).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between font-mono">
                      <span className="text-slate-500">Transaction ID:</span>
                      <span className="font-bold text-indigo-600">{gatewayTransactionResult.transactionId}</span>
                    </div>
                  </div>

                  <div className="pt-4 border-t border-slate-200 flex justify-between items-center bg-slate-50 -mx-6 -mb-6 p-4 rounded-b-2xl">
                    <span className="text-xs font-black uppercase text-slate-900">Balance Settled Complete:</span>
                    <span className="text-xl font-black text-indigo-600">${gatewayTransactionResult.price}</span>
                  </div>
                </div>

                {/* ACTION TRIGGERS IN SUCCESS VIEW */}
                <div className="space-y-2 pt-2">
                  <button 
                    id="btn-print-invoice-simulation"
                    onClick={() => {
                      setIsReceiptPrintSimulated(true);
                      setTimeout(() => setIsReceiptPrintSimulated(false), 2000);
                    }}
                    className="w-full py-2.5 bg-white/5 hover:bg-white/10 text-slate-300 font-bold uppercase text-[11px] rounded-xl border border-white/10 transition-all flex items-center justify-center gap-1.5 focus:outline-none cursor-pointer"
                  >
                    <span>{isReceiptPrintSimulated ? '📥 Copying invoice PDF to memory...' : '📥 Format Invoice Receipt (PDF)'}</span>
                  </button>

                  <button
                    id="btn-close-gateway-dashboard"
                    onClick={() => {
                      setIsGatewayOpen(false);
                      setActiveTab('dashboard'); // Redirect safely to customer dashboard portal instantly!
                    }}
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase text-xs tracking-widest rounded-xl transition-all shadow-md cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <span>Inspect active project monitor inside Client Portal</span>
                    <ChevronRight className="w-4 h-4" />
                  </button>
                </div>
            ...
            {gatewayStep === 'failed' && (
              <div className="py-6 space-y-6 text-center animate-fadeIn">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-black text-red-400 uppercase tracking-tight">Authorization Refused</h3>
                  <p className="text-xs text-slate-400 max-w-xs mx-auto">The transaction handshake was declined by your payment network. Please ensure details are correct and retry.</p>
                </div>

                <div className="flex gap-3">
                  <button
                    onClick={() => setIsGatewayOpen(false)}
                    className="flex-1 py-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold uppercase transition-all cursor-pointer"
                  >
                    Close gateway
                  </button>
                  <button
                    onClick={() => setGatewayStep('input')}
                    className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-md cursor-pointer"
                  >
                    Try Reconfiguring
                  </button>
                </div>
